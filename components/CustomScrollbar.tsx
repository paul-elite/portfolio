...
export default function CustomScrollbar({
  ...
  return (
    <div ref={containerRef} className={`relative flex flex-col overflow-hidden ${className}`}>
      ...
      <div
        ref={contentRef}
        className={`flex-1 min-h-0 overflow-y-auto ${position === 'right' ? 'pr-4' : 'pl-4'}`}
      ...
      {showScrollbar && (
        <div
          ref={trackRef}
          className={`absolute top-0 ${position === 'right' ? 'right-0' : 'left-0'} h-full cursor-pointer`}
        ...
        >
          ...
          <div
            ref={thumbRef}
            className={`absolute ${position === 'right' ? 'right-0' : 'left-0'} rounded-full cursor-grab ${isDragging ? 'cursor-grabbing' : ''}`}
            style={{
              height: thumbHeight,
              width: thumbWidth,
              top: thumbTop,
